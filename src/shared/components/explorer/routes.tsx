...
const PRESERVED = import.meta.glob('/src/pages/(_app|404).{jsx,tsx}', {
  eager: true,
})
...
type Route = RouteObject &
  Partial<{ key: string; modal: boolean; pathname: string }>

const preserved = Object.keys(PRESERVED).map((key) =>
  key.replace(...patterns.route),
)
const routes = generateRegularRoutes<Route, {}>(
  { ...ROUTES, ...MODALS },
  (_, key) => ({
...
const Tree = ({
  ...
  <ul className="flex select-none flex-col text-sm font-medium">
    {routes?.map((route) => {
      const pathname = route.pathname?.startsWith('/')
        ? route.pathname
        : `/${route.pathname}`
      const path = pathname!
        .replace(/\([\w-]+\)\/|\/?_layout/g, '')
      ...
      const disabled =
        preserved.includes(route?.id!) || route.id?.includes('_layout')
      ...
      return (
        <li key={route?.id + route?.path!} className="flex flex-col">
          {route?.children?.length ? (
            <details open={true}>
              <summary
                className="cursor-pointer space-x-2 py-2.5"
                style={{ paddingLeft }}
              >
                ...
                <span>
                  {route.key?.includes('_layout')
                    ...
                    : route.id ||
                      route?.children?.[0]?.id?.split('/')?.at(-2) ||
                      route.path}
                </span>
              </summary>
              <Tree
                routes={sort(
                  route.pathname?.includes('_layout')
                    ? [
                        { id: route.key, pathname: route.pathname },
                        ...route.children,
                      ]
                    : route.children,
                )}
            ...
            <Link
              ...
              className={classNames(
                ...
                location.pathname === path && !pathname.includes('_layout')
                  ? 'bg-slate-50'
                  : '',
                disabled
          ...
          routes={[
            {
              ...
              children: sort([
                ...routes,
                ...preserved.map((x) => ({ id: x, pathname: x })),
              ]),
            },
          ]}
          depth={1}
